...
class NewsProvider {
  ...
  Future<ListData> searchArticles(String keyword, int page, {int pageSize = 20}) async {
    ...
    ListData articles = ListData([], 0, false);
    await ApiService().searchArticles(keyword, page, pageSize: pageSize).then((response) {
      if (response.statusCode == 200) {
        Iterable data = jsonDecode(response.body)['articles'];
        articles = ListData(
          data.map((e) => News.fromJson(e)).toList(),
